...
class Model_app extends CI_Model{
    ...
    public function all()
    {
        ...
        $hasil = $this->db->get('tbl_penjualan_header');
        if($hasil->num_rows() > 0){
            return $hasil->result();
        } else {
            return false;
    ...
    {
        //Get all invoices from Invoices table
        $hasil = $this->db->get('tbl_sj_header');
        if($hasil->num_rows() > 0){
            return $hasil->result();
        } else {
            return false;
    ...
    function getAllDataSJ(){
        return $this->db->query("SELECT
                a.kd_penjualan,
                a.tanggal_penjualan,
                a.total_harga,
                a.kd_pelanggan,
                (select count(kd_penjualan) as jum from tbl_sj_detail where kd_penjualan=a.kd_penjualan) as jumlah
                from tbl_sj_header a
                ORDER BY a.kd_penjualan DESC
        ")->result();
    }
...
//                select a.kd_barang,a.qty,b.nm_barang,b.harga,b.ukuran,b.quality,b.type
...
//    }
        ...
        function getBarangPenjualan($id){
        return $this->db->query("SELECT * from tbl_penjualan_detail
                where kd_penjualan = '$id'")->result();
    }

        function getBarangSJ($id){
        return $this->db->query("SELECT * from tbl_sj_detail
                where kd_penjualan = '$id'")->result();
    }
...
//    }
   ...
   function getLapPenjualan($tgl_awal,$tgl_akhir){
        return $this->db->query("SELECT
                ...
                a.total_harga,
                a.kd_pelanggan,
                (select sum(qty) as jum from tbl_penjualan_detail where kd_penjualan=a.kd_penjualan) as jumlah
                from tbl_penjualan_header a
                where tanggal_penjualan between '$tgl_awal' and '$tgl_akhir'
                ")->result();
    }

    function getCustPintar($tgl_awal,$tgl_akhir){
        return $this->db->query("SELECT * from tbl_penjualan_header a
                join tbl_penjualan_detail b on a.kd_penjualan=b.kd_penjualan
                where tanggal_penjualan between '$tgl_awal' and '$tgl_akhir'
                ")->result();
    }
